...
public class EventTpArg extends Argument {

    public EventTpArg(CommandManager manager) {
        super(manager, Collections.singletonList("tp"));
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(CC.t("&cUsage: /event <sumo|tnttag> tp"));
            return;
        }
        ...
        if (!player.hasPermission(permission)) {
            sender.sendMessage(CC.t("&cYou don't have permission to teleport to this event."));
            return;
        }
        ...
        teleportToEvent(eventType, player);
        player.sendMessage(CC.t("&aTeleported to " + eventType.getColor() + eventType.getName() + " event."));
    }
    ...
    private void teleportToEvent(EventType eventType, Player player) {
        switch (eventType) {
            case SUMO -> player.teleport(SoupPvP.getInstance().getSumoHandler().getSpectatorSpawn());
            case TNTTAG -> player.teleport(SoupPvP.getInstance().getTntTagHandler().getSpectatorSpawn());
        }
    }
}
