package kami.gg.souppvp.leaderboard.command;

import kami.gg.souppvp.leaderboard.LeaderboardType;
import kami.gg.souppvp.leaderboard.menu.LeaderboardMenu;
...
public class LeaderboardCommand extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        try {
            type = LeaderboardType.valueOf(typeArg);
        } catch (IllegalArgumentException e) {
            sendMessage(sender, "&cInvalid leaderboard type. Available: " + 
                Arrays.toString(LeaderboardType.values()));
            return;
        }

        if (args.length == 1) {
            new LeaderboardMenu(type).openMenu(player);
        }
    }
}