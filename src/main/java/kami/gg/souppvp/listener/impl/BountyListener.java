    ...
    public void onPlayerDeathEvent(PlayerDeathEvent event) {
        ...
        if (!(event.getEntity().equals(event.getEntity().getKiller()))) {
            if (victimProfile.getBounty() > 0) {
                Bukkit.broadcastMessage(CC.t("&a" + event.getEntity().getKiller().getName() + "&e has claimed the &a" + victimProfile.getBounty() + "&e credit bounty from &a" + event.getEntity().getName() + "&e."));
